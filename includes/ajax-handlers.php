...
class CODVerifierAjax {
    ...
    public function __construct() {
        ...
        // Webhook handler - MODIFIED to process payment.captured
    ...
    public function create_payment_link() {
        ...
        if (!$woocommerce_order_id) {
            ...
            if (WC()->cart && !WC()->cart->is_empty()) {
                ...
                $woocommerce_order_id = 'temp_' . time() . '_' . wp_generate_password(8, false);
                WC()->session->set('cod_verifier_temp_order_id', $woocommerce_order_id);
            } else {
                error_log('COD Verifier Error: WooCommerce Order ID not found for Razorpay Order creation.');
                wp_send_json_error(__('Internal error: Cannot create payment order.', 'cod-verifier'));
                return;
            }
        }
        ...
        if (isset($_SESSION['cod_otp_phone']) && !empty($_SESSION['cod_otp_phone'])) {
        ...
        if (isset($result['id'])) {
            // Success: Return Order details for frontend Checkout JS
            wp_send_json_success(array(
                'order_id' => $result['id'],
    ...
    public function verify_token_payment_server() {
        ...
        if (hash_equals($generated_signature, $razorpay_signature)) {
            // Signature valid - set session flag
            if (!session_id()) {
                session_start();
            }
            $_SESSION['cod_token_paid'] = true;
            
            wp_send_json_success(__('Payment successfully verified.', 'cod-verifier'));
        } else {
            error_log('COD Verifier Server Verification Error: Signature mismatch for Payment ID: ' . $payment_id . ' (Order ID: ' . $order_id . ').');
            wp_send_json_error(__('Payment verification failed. Invalid signature.', 'cod-verifier'));
        }
    }
    
    // MODIFIED: Enhanced webhook handler for payment.captured
    public function handle_webhook() {
        ...
        if (isset($data['event']) && $data['event'] === 'payment.captured') {
            ...
            if ($woo_order_id) {
                ...
                error_log('COD Verifier Webhook Error: WooCommerce Order ID not found in webhook payload for payment ' . $payment_id . ' (Razorpay Order ID: ' . $razorpay_order_id . ').');
            }
